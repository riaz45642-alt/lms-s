...
import { workbookYears, workbookSubjects, ALL_WORKBOOKS } from '../data/workbooksData'
...
export default function Workbooks() {
  ...
  return (
    <>
      <PageHero eyebrow="Structured resources" title="Workbooks" subtitle="Multi-week workbooks that guide students through a topic step by step, with daily practice pages built in." />
        ...
        <div className="wrap">
          <div className="filters-bar">
            <div className="search-lg">
              ...
              <input placeholder="Search workbooks…" value={query} onChange={(e) => setQuery(e.target.value)} />
            </div>
          </div>
          <div className="filters-bar">
          ...
          {loading ? (
            ...
            <EmptyState icon="📘" title="No workbooks found" message="Try adjusting your filters or search term." actionLabel="Clear filters" onAction={resetFilters} />
          ) : (
            <>
              <div className="grid-cards">
                {pageItems.map((w) => (
                  <div className="wcard" key={w.id}>
                    <div className="wthumb" style={{ background: w.badge === 'free' ? '#eef6fe' : '#fff7e6', cursor: 'pointer' }} onClick={() => navigate(`/workbooks/${w.id}`)}>
                    ...
                    <div className="wbody">
                      <h4 style={{ cursor: 'pointer' }} onClick={() => navigate(`/workbooks/${w.id}`)}>{w.title}</h4>
                      <div className="wmeta">
                        <span className="lvl">{w.year} · {w.weeks} weeks</span>
                      </div>
                      <div className="wcard-actions">
                        <button className="btn btn-ghost" onClick={() => navigate(`/workbooks/${w.id}`)}>Preview</button>
                        <button className="btn btn-primary" onClick={() => navigate(`/workbooks/${w.id}`)}>Get workbook</button>
